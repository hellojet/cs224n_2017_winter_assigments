...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable(name='W_x', shape=(self.input_size, self.state_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=1))
            W_h = tf.get_variable(name='W_h', shape=(self.state_size, self.state_size), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=2))
            b = tf.get_variable(name='b', shape=(self.state_size,), dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer(seed=3))
